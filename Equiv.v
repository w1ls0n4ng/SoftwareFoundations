...
Proof.
  split; intros.
  - inversion H. inversion H5. rewrite <- H8. apply H2.
  - apply E_Seq with st'.
    + apply H.
    + apply E_Skip.
Qed.
...
Proof.
  split; intros.
  - exfalso. apply (while_true_nonterm b c st st' H). apply H0.
  - inversion H0; subst.
    + discriminate.
...
Proof.
  ...
  - inversion H0; subst.
    ...
    rewrite Hx at 2. apply E_Asgn. rewrite <- H. reflexivity.
  - inversion H0; subst. rewrite <- H. rewrite t_update_same. apply E_Skip.
Qed.
...
Proof.
  ...
  { unfold bequiv, cequiv. intros.
    remember <{ if b then c1 else c2 end }> as Cif. induction H2; inversion HeqCif; subst.
    - apply E_IfTrue.
      + rewrite <- H. apply H2.
      + rewrite <- H0. apply H3.
    - apply E_IfFalse.
...
Proof.
  ...
  assert (H0 : empty_st =[ while true do skip end ]=> empty_st).
  { apply H. apply E_Skip. }
  remember <{ while true do skip end }> as W.
  induction H0; inversion HeqW.
