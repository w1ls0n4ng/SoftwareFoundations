...
Theorem hoare_pre_false : forall (P Q : Assertion) c,
  ...
  {{P}} c {{Q}}.
Proof.
  unfold valid_hoare_triple. intros. unfold not in H. exfalso. apply H with st. apply H1.
Qed.
(** [] *)
...
Theorem hoare_asgn_wrong : exists a:aexp,
  ~ {{ True }} X := a {{ X = a }}.
Proof.
  exists (<{X + 1}>). unfold not, valid_hoare_triple. intros.
  specialize H with empty_st (X !->1).
  assert (empty_st =[ X := X + 1 ]=> (X !-> 1)) as ex.
  { apply E_Asgn. reflexivity. }
  apply H in ex.
  - simpl in ex. unfold t_update in ex. simpl in ex. inversion ex.
  - apply I.
Qed.
...
Proof.
  unfold swap_program.
  eapply hoare_seq with (Q := (Z <= Y)%assertion).
  - eapply hoare_seq with (Q := (Z <= X)%assertion).
    + asgn_auto.
    + asgn_auto.
  - asgn_auto.
Qed.
...
Proof.
  unfold valid_hoare_triple. intros P Q b c HTrue HFalse st st' Hc HP.
  inversion Hc; subst.
  - eauto.
  - eauto.
Qed.
...
Proof.
  ...
  - eapply hoare_consequence_pre.
    + apply hoare_skip.
    + assertion_auto''. destruct H. apply eq_true_negb_classical in H0.
      apply eqb_eq in H0. rewrite H0 in H. rewrite add_0_r in H.
      assumption.
Qed.
