...
Proof. 
  verify.
  - rewrite <- H. rewrite lemma1. symmetry in H0. apply andb_true_eq in H0.
    destruct H0. symmetry in H0, H1. apply negb_true_iff in H0, H1. apply eqb_neq in H0, H1.
    assert (1 + (min (st X) (st Y) - 1) = min (st X) (st Y)).
    { destruct (min (st X) (st Y)) eqn:HH.
      + apply lemma2 in HH. destruct HH; congruence.
      + lia. }
    rewrite <- add_assoc. rewrite H2. reflexivity.
  - apply andb_false_iff in H0. destruct H0;
    apply negb_true_iff in H0; rewrite negb_involutive in H0; apply eqb_eq in H0;
      rewrite <- H; rewrite H0; lia.
Qed.
