...
Proof.
  ...
  induction c; intros.
  - eapply H_Consequence_post. apply H_Skip. constructor.
  - eapply H_Consequence_pre. apply H_Asgn. constructor.
  - eapply H_Seq with (assert_of_Prop True). all:auto.
  - eapply H_If. all:auto.
  - eapply H_Consequence_pre with (assert_of_Prop True). all: eauto.
Qed.
(** [] *)
...
Proof.
  ...
  - eapply H_Seq with (assert_of_Prop False). all:auto.
  - eapply H_If; eapply H_Consequence_pre. all:auto. all:(intros; inversion H; inversion H0).
  - eapply H_Consequence_post. eapply H_While.
...
Proof.
  induction 1; unfold valid; intros; try solve [ inversion H; subst; eauto ].
  remember (CWhile b c) as original_command eqn:Horig.
  induction H; inversion Horig; subst; eauto.
Qed.
...
Proof.
  ...
  1:{
    ...
    * apply H_Skip.
    * intros. eapply HT. 2:apply H. apply E_Skip.
  }
  1:{
    eapply H_Consequence_pre.
    * apply H_Asgn.
    * intros. eapply HT. 2:apply H. apply E_Asgn. reflexivity.
  }
  1:{
    Hint Unfold wp : core.
    eapply H_Seq with (Q:=wp c2 Q).
    - apply IHc2. unfold wp. intros. apply H0. apply H.
    - apply IHc1. unfold wp. intros. eapply HT. eapply E_Seq.
      all: eauto.
  }
  2:{
    ...
    * intros. eauto.
    * intros. simpl in H. unfold wp in H. apply H. apply E_WhileFalse.
      apply Bool.not_true_is_false. apply H.
  }
  1:{
    eapply H_If.
    - apply IHc1. intros. destruct H0. apply HT with (st:=st).
      * apply E_IfTrue; auto.
      * assumption.
    - apply IHc2. intros. destruct H0. apply HT with (st:=st).
      * apply E_IfFalse; auto. unfold not in H1. simpl in H1. apply Bool.not_true_is_false.
        unfold not. apply H1.
      * assumption.
  }
Qed.
