...
Proof.
  intros. induction H.
    - simpl. apply H0.
    - simpl. rewrite -> ev_Even_iff. rewrite -> ev_Even_iff in IHev. unfold Even in IHev. 
      destruct IHev. unfold Even. exists (S x). simpl. rewrite -> H1. reflexivity.
Qed.
...
(** - + [R 1 1 2] is provable by applying c2, c3 and c1 accordingly.
      + [R 2 2 6] is not provable as there is no way of reducing the third argument to 0.

    - No, because it only swipes the value of the first and two arguments, you can just
      use c2 or c3 to change their value without swiping.

    - No, because in order to prove this proposition, you must reduce all the values of the
      arguments to 0, applying c2 and c3 after applying c4 does not make any difference.
*)
