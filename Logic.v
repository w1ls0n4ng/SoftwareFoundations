...
Proof.
  intros. apply conj. induction n.
  - reflexivity.
  - simpl in H. discriminate H.
  - induction m.
    + reflexivity.
    + simpl. rewrite -> PeanoNat.Nat.add_succ_r in H. discriminate H.
Qed.
...
Theorem contrapositive : forall (P Q : Prop),
  (P -> Q) -> (~Q -> ~P).
Proof.
  intros. unfold not in H0. unfold not. intros. apply H0. apply H. apply H1.
Qed.
...
Theorem de_morgan_not_or : forall (P Q : Prop),
    ~ (P \/ Q) -> ~P /\ ~Q.
Proof.
  intros. unfold not in H. apply Decidable.not_or_iff in H. destruct H as [HP HQ]. apply conj.
  - unfold not. apply HP.
  - unfold not. apply HQ.
Qed.
(** [] *)
...
Proof.
  intros. unfold not. intros. destruct H0 as [x E]. apply E. apply H.
Qed.
(** [] *)
...
Proof.
   split.
   - intros. destruct H as [x PxQx]. destruct PxQx as [Px | Qx].
     + left. exists x. apply Px.
     + right. exists x. apply Qx.
  - intros. destruct H as [Px | Qx].
    + destruct Px as [x Px]. exists x. left. apply Px.
    + destruct Qx as [x Qx]. exists x. right. apply Qx.
Qed.
...
Proof.
  ...
  { induction l as [|x l' IHl'].
    ...
    - simpl. intros [H | H].
      ...
      + destruct IHl'. apply H. destruct H0. exists x0. split. apply H0. right. apply H1. }
  { intros [x [H0 H1]]. induction l as [|x' l' IHl'].
...
Proof.
  split.
  - intros. apply conj.
    + rewrite -> andb_commutative in H. rewrite -> (andb_true_elim2 _ _ H). reflexivity.
    + rewrite -> (andb_true_elim2 _ _ H). reflexivity.
...
Proof.
  ...
  - generalize dependent y. induction x.
    ...
    + destruct y.
      * reflexivity.
      * simpl. intros. apply IHx. intros Hs. unfold not in H. apply H. rewrite -> Hs. reflexivity.
Qed.
...
Proof.
  ...
  - induction l as [| a' l' IHl'].
    + reflexivity.
    + simpl. intros. apply andb_true_iff in H. destruct H as [H1 H2]. split.
      * apply H1.
      * apply IHl'. apply H2.
  - induction l as [| a' l' IHl'].
    + intros. reflexivity.
    + simpl. intros. apply andb_true_iff. destruct H as [H1 H2]. apply IHl' in H2. split.
      * apply H1.
      * apply H2.
Qed.
