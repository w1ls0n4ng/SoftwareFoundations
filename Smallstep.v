...
Proof.
  intros. induction H.
  - apply multi_refl.
  - eapply multi_trans. 1:eapply multi_trans.
    + apply multistep_congr_1 with (t2:=t2) in IHeval1. apply IHeval1.
    + apply multistep_congr_2 with (v1:=(C n1)) in IHeval2. 1:apply IHeval2. constructor.
...
Proof.
  unfold normal_form_of. intros. destruct H. apply nf_same_as_value in H0. induction H.
  - inversion H0; subst. exists n. split.
    + reflexivity.
    + apply E_Const.
  - destruct IHmulti.
    + apply H0.
    + inversion H0; subst. exists n. split.
      * reflexivity.
      * destruct H2. inversion H2. apply step__eval with (n:=x0) in H; assumption.
Qed.
