...
Proof.
  ...
  assert (H3: z :: l = y :: l -> z = y). { intros. injection H as Help0. apply Help0. }
  apply H3. rewrite <- H0. rewrite -> H2. reflexivity.
Qed.
...
Proof.
  ...
  - intros. destruct m.
    + discriminate H.
    + f_equal. apply IHn. simpl in H. injection H as H0. rewrite <- plus_n_Sm in H0.
      rewrite <- plus_n_Sm in H0. injection H0 as H0. apply H0.
Qed.
(** [] *)
...
Proof.
  ...
  - intros. injection H as H0 H1. rewrite <- H0. rewrite <- H1. reflexivity.
  - intros. destruct x. simpl in H. destruct (split l). injection H as H0 H1.
    rewrite <- H0. rewrite <- H1. simpl. rewrite -> IHl.
    + reflexivity.
    + reflexivity.
Qed.
